...
class Sum{
	...
	/*
	메서드 호출시 전될다는 인수의 값을 누적해서 멤버변수에 더한다.
...
/*
 ...
 상속해야 한다면 Runnable 인터페이스를 구현해서 쓰레드 클래스를
 정의해야한다.
 */
class AdderThread extends Sum implements Runnable {
	...
	/*
	 ...
	 호출하여 누적합을 계산한다. start가 1이고 end가 10이라면 1~10까지
	 합을 쓰레드를 통해 계산하게된다.
	 */
	@Override
	public void run() {
...
public class Ex04Runnable {
	...
	public static void main(String[] args) {
		...
		//start()를 통해 쓰레드를 시작한다.ㄴ
		thread1.start();
		thread2.start();
		
		try {
			/*
			 생성된 쓰레드가 하나의 직업을 완료하기를 기다리는 기능을
