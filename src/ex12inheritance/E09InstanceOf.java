...
public class E09InstanceOf {
	...
	/*
	 ...
	 모두 받을 수 있다.
	 -Box타입으로 인수를 받게되면 자동형변환(엽캐스팅) 되므로 자식 클래스의
	 멤버메서드는 호출할 수 없게도니다
	 -이떄 자식의 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)이
	 필요하다.
	 -instanceof 연산자를 통해 자식 타입으로 형변환이 가능한지 판단한 후
	 true를 반환하면 다운캐스팅을 진행한 후 메서드를 호출할 수 있게된다.
	 */
...
//	static void wrapBox(GoldPaperBox b) {}
	...
	/*
	 상속관계가 더 깊어진다면 메서드는 점점 늘어나게되며 코드는 더욱 더
